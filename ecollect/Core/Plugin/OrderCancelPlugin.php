...
class OrderCancelPlugin
{
    ...
    protected function salesOrderBeforeCancel()
    {
        ...
        if ($isValidBasicData){
            ...
            if ($isValidaData){
                ...
                $accessToken = $this->_scopeConfig->getValue(\ecollect\Core\Model\Core::XML_PATH_ACCESS_TOKEN, \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
                ...
                if ($response['status'] == 200) {
                    $this->messageManager->addSuccessMessage(__('Cancellation made by e-Collect'));
                } else {
                    $this->messageManager->addErrorMessage(__('Failed to make the cancellation by e-Collect'));
    ...
    protected function checkCancelationBasicData($paymentID, $paymentMethod)
    {
        ...
        if (!$refundAvailable) {
            $this->messageManager->addWarningMessage(__('e-Collect cancellation is disabled. The cancellation will be made through Magento'));
            return false;
        }

        if ($refundAvailable && $paymentID == null){
            $this->messageManager->addWarningMessage(__('The cancellation will be made through Magento. It was\'t possible to cancel on ecollect'));
            return false;
        }

        if (!($paymentMethod == 'ecollect_standard' || $paymentMethod == 'ecollect_custom')) {
            $this->messageManager->addWarningMessage(__('Order payment wasn\'t made by e-Collect. The cancellation will be made through Magento'));
            return false;
        }
    ...
    {
        throw new \Magento\Framework\Exception\LocalizedException(new \Magento\Framework\Phrase('e-Collect - Cancellations not made'));
    }
}