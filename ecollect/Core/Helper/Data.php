...
{
    /**
     *path to ecollect custom ticket active
     */
    const XML_PATH_MERCADOPAGO_TICKET_ACTIVE = 'payment/ecollect_customticket/active';


    /**
     *path to access token config
     */
    const XML_PATH_ACCESS_TOKEN = 'payment/ecollect_custom/access_token';
    /**
     *path to public config
     */
    const XML_PATH_PUBLIC_KEY = 'payment/ecollect_custom/public_key';
    /**
     *path to ecollect custom active config
     */
    const XML_PATH_MERCADOPAGO_CUSTOM_ACTIVE = 'payment/ecollect_custom/active';


    /**
     *path to client id config
     */
    const XML_PATH_CLIENT_ID = 'payment/ecollect_standard/client_id';
    /**
     *path to client secret config
     */
    const XML_PATH_CLIENT_SECRET = 'payment/ecollect_standard/client_secret';


    /**
     *path to ecollect country config
     */
    const XML_PATH_COUNTRY = 'payment/ecollect/country';
    /**
     *path to payment calculator available
     */
    const XML_PATH_CALCULATOR_AVAILABLE = 'payment/ecollect/calculalator_available';
    /**
     *path to the list of pages on which to display the calculator
     */
    const XML_PATH_CALCULATOR_PAGES = 'payment/ecollect/show_in_pages';
    /**
     *path to ecollect refund available config
     */
    const XML_PATH_REFUND_AVAILABLE = 'payment/ecollect/refund_available';
    /**
     *path to maximum days refund config
     */
    const XML_PATH_MAXIMUM_DAYS_REFUND = 'payment/ecollect/maximum_days_refund';
    /**
     *path to maximum partial refunds config
     */
    const XML_PATH_MAXIMUM_PARTIAL_REFUNDS = 'payment/ecollect/maximum_partial_refunds';
    /**
     *path to order status refunded config
     */
    const XML_PATH_ORDER_STATUS_REFUNDED = 'payment/ecollect/order_status_refunded';
    /**
     *path to use successpage mp config
     */
    const XML_PATH_USE_SUCCESSPAGE_MP = 'payment/ecollect/use_successpage_mp';
    /**
     *path to sponsor id config
     */
    const XML_PATH_SPONSOR_ID = 'payment/ecollect/sponsor_id';

    const XML_PATH_CONSIDER_DISCOUNT = 'payment/ecollect/consider_discount';
    ...
    const XML_PATH_ENTITY_ODE = 'payment/ecollect_standard/entityCode';
    const XML_PATH_API_KEY = 'payment/ecollect_standard/apiKey';
    const XML_PATH_SRV_CODE = 'payment/ecollect_standard/srvCode';
    const XML_PATH_SRV_CURRENCY = 'payment/ecollect_standard/srvCurrency';
    const XML_PATH_WDSL_PRODUCTION = 'payment/ecollect_standard/wdslProduction';
    const XML_PATH_WDSL_TEST = 'payment/ecollect_standard/wdslTest';
    const XML_PATH_TESTMODE = 'payment/ecollect_standard/testmode';
     ...
     *
     * @var \ecollect\Core\Logger\Logger
     */
    protected $_mpLogger;
     ...
     * @param \ecollect\Core\Logger\Logger                      $logger
    ...
    public function __construct(
        ...
        \ecollect\Core\Logger\Logger $logger,
     ...
     * @param null   $array
     */
    public function log($message, $name = "ecollect", $array = null)
    {
        //load admin configuration value, default is true
        $actionLog = $this->scopeConfig->getValue('payment/ecollect/logs', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        if (!$actionLog) {
            return;
        }
    ...
    public function getApiInstance()
    {
        ...
        /*if ($params == 1) {
            $api = new \ecollect\Core\Lib\Api(func_get_arg(0));
            $api->set_platform(self::PLATFORM_OPENPLATFORM);
        } else {
            if ($params == 3) {
                $api = new \ecollect\Core\Lib\Api(func_get_arg(0), func_get_arg(1));
        ...
        $api = new \ecollect\Core\Lib\Api();
        $api->set_platform(self::PLATFORM_OPENPLATFORM);
        if ($this->_switcher->getWebsiteId() != 0) {
            if ($this->scopeConfig->getValue('payment/ecollect_standard/sandbox_mode', \Magento\Store\Model\ScopeInterface::SCOPE_WEBSITE, $this->_switcher->getWebsiteId())) {
                $api->sandbox_mode(true);
            }
        } else {
            if ($this->scopeConfig->getValue('payment/ecollect_standard/sandbox_mode')) {
                $api->sandbox_mode(true);
            }
        }
    ...
    public function getModuleVersion()
    {
        ...
        return "1.5.3";//return $magentoPackages['ecollect/magento2-plugin']['version'];
    }
    ...
    public function getAnalyticsData($order)
    {
        ...
        if ($order->getPayment()->getData('method')) {
            ...
            if ($methodCode == \ecollect\Core\Model\Custom\Payment::CODE) {
                $analyticsData['public_key'] = $this->scopeConfig->getValue('payment/ecollect_custom/public_key', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
            } elseif ($methodCode == \ecollect\Core\Model\Standard\Payment::CODE) {
                $analyticsData['analytics_key'] = $this->scopeConfig->getValue(\ecollect\Core\Helper\Data::XML_PATH_CLIENT_ID, \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
                ...
                //$analyticsData['analytics_key'] = $this->getClientIdFromAccessToken($this->scopeConfig->getValue(\ecollect\Core\Helper\Data::XML_PATH_ACCESS_TOKEN, \Magento\Store\Model\ScopeInterface::SCOPE_STORE));
