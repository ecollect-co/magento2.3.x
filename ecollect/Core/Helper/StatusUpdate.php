<?php
namespace ecollect\Core\Helper;

/**
 * Class StatusUpdate
 *
 * @package ecollect\Core\Helper
    ...
    extends \Magento\Payment\Helper\Data
{
     ...
     */
    public $_statusUpdatedFlag = false;
    /**
     * @var \Magento\Sales\Model\OrderFactory
     */
    protected $_orderFactory;
    /**
     * @var \ecollect\Core\Helper\Message\MessageInterface
     */
    ...
    public function __construct(
        \ecollect\Core\Helper\Message\MessageInterface $messageInterface,
        ...
        \ecollect\Core\Model\Core $coreHelper,
    ...
    {
        return $this->scopeConfig->getValue('payment/ecollect/order_status_refunded');
    }
    ...
    {
        file_put_contents("cron.html","setStatusUpdated arr ".json_encode($notificationData)."<br>order ".json_encode($order),FILE_APPEND);
        ...
        $fflag = ( $this->_statusUpdatedFlag) ? "true":"false";
        file_put_contents("cron.html","setStatusUpdated _statusUpdatedFlag $fflag arr ".json_encode($notificationData)."<br>",FILE_APPEND);
        if (!is_null($order->getPayment()) && $order->getPayment()->getAdditionalInformation('second_card_token')) {
            file_put_contents("cron.html","existe pago, retorna false<br>",FILE_APPEND);
            $this->_statusUpdatedFlag = false;

            return;
        }
        if ($status == $currentStatus && $statusDetail == $currentStatusDetail) {
            file_put_contents("cron.html","$status == $currentStatus y $statusDetail == $currentStatusDetail son iguales<br>",FILE_APPEND);
    ...
    public function getStatusOrder($status, $statusDetail, $isCanCreditMemo)
    {
        switch ($status) {
            case 'approved': {
                $status = $this->scopeConfig->getValue('payment/ecollect/order_status_approved', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
                if ($statusDetail == 'partially_refunded' && $isCanCreditMemo) {
                    $status = $this->scopeConfig->getValue('payment/ecollect/order_status_partially_refunded', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
                }
                break;
            }
            case 'refunded': {
                $status = $this->scopeConfig->getValue('payment/ecollect/order_status_refunded', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
                break;
            }
            case 'in_mediation': {
                $status = $this->scopeConfig->getValue('payment/ecollect/order_status_in_mediation', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
                break;
            }
            case 'cancelled': {
                $status = $this->scopeConfig->getValue('payment/ecollect/order_status_cancelled', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
                break;
            }
            case 'rejected': {
                $status = $this->scopeConfig->getValue('payment/ecollect/order_status_rejected', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
                break;
            }
            case 'chargeback': {
                $status = $this->scopeConfig->getValue('payment/ecollect/order_status_chargeback', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
                break;
            }
            default: {
                $status = $this->scopeConfig->getValue('payment/ecollect/order_status_in_process', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
            }
        }
    ...
    protected function _getLastPaymentIndex($payments, $status)
    {
        ...
        usort($dates, ['ecollect\Core\Controller\Notifications\Standard', "_dateCompare"]);
        if ($dates) {
    ...
    public function setStatusOrder($payment)
    {
        ...
        ////file_put_contents("log.html","before _getOrder ".$payment["external_reference"]."<br>",FILE_APPEND);
        $order = $this->_coreHelper->_getOrder($payment["external_reference"]);
        ////file_put_contents("log.html","AFTER _getOrder <br>",FILE_APPEND);
        ...
        ////file_put_contents("log.html","before getMessage <br>",FILE_APPEND);
        $message = $this->getMessage($status, $payment);
        ////file_put_contents("log.html","message = $message <br>",FILE_APPEND);
        if ($this->isStatusUpdated()) {
            return ['text' => $message, 'code' => \ecollect\Core\Helper\Response::HTTP_OK];
        }

        ////file_put_contents("log.html","before _updateStatus <br>",FILE_APPEND);
        //if state is not complete updates according to setting
        $this->_updateStatus($order, $status, $message, $statusDetail);
        ////file_put_contents("log.html","AFTER _updateStatus before order save<br>",FILE_APPEND);

        $statusSave = $order->save();
        ////file_put_contents("log.html","statusSave $statusSave <br>",FILE_APPEND);
        $this->_dataHelper->log("Update order", 'ecollect.log', $statusSave->getData());
        $this->_dataHelper->log($message, 'ecollect.log');
        ...
        try {
            ...
            $this->_dataHelper->log("erro in set order status: " . $e, 'ecollect.log');
            ////file_put_contents("log.html","Exception  <br>",FILE_APPEND);

            return ['text' => $e, 'code' => \ecollect\Core\Helper\Response::HTTP_BAD_REQUEST];
        }
    }
     ...
     * @param $statusHelper \ecollect\Core\Helper\StatusUpdate
     * @param $status
     * @param $message
     * @param $statusDetail
     */
    ...
    public function updateOrder($data, $order = null)
    {
        ...
        ////file_put_contents("log.html","INSIDE updateOrder<br>",FILE_APPEND);

        $this->_dataHelper->log("Update Order", 'ecollect-notification.log');
        if (!$this->isStatusUpdated()) {
            try {
                if (!$order) {
                ...
                ////file_put_contents("log.html","before paymentOrder save<br>",FILE_APPEND);
                $paymentStatus = $paymentOrder->save();
                $this->_dataHelper->log("Update Payment", 'ecollect.log', $paymentStatus->getData());

                ////file_put_contents("log.html","before order save<br>",FILE_APPEND);
                $statusSave = $order->save();
                $this->_dataHelper->log("Update order", 'ecollect.log', $statusSave->getData());
            } catch (\Exception $e) {
                ////file_put_contents("log.html","exception in StatusUpdate<br>",FILE_APPEND);
                $this->_dataHelper->log("erro in update order status: " . $e, 'ecollect.log');
                $this->getResponse()->setBody($e);

                //if notification proccess returns error, ecollect will resend the notification.
                $this->getResponse()->setHttpResponseCode(\ecollect\Core\Helper\Response::HTTP_BAD_REQUEST);
            }
        }
    }
