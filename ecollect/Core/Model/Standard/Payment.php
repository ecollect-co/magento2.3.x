...
namespace ecollect\Core\Model\Standard;

/**
 * Class Payment
 *
 * @package ecollect\Core\Model\Standard
    ...
    extends \Magento\Payment\Model\Method\AbstractMethod
{
     ...
     */
    const CODE = 'ecollect_standard';

    /**
     * define URL to go when an order is placed
     */
    const ACTION_URL = 'ecollect/standard/pay';
     ...
     * @var string
     */
    protected $_infoBlockType = 'ecollect\Core\Block\Info';
    ...
    {
        /*$client_id = $this->_scopeConfig->getValue(\ecollect\Core\Helper\Data::XML_PATH_CLIENT_ID, \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $client_secret = $this->_scopeConfig->getValue(\ecollect\Core\Helper\Data::XML_PATH_CLIENT_SECRET, \Magento\Store\Model\ScopeInterface::SCOPE_STORE);*/

        $mp = $this->_helperData->getApiInstance();

        $pref = $this->makePreference();
        $this->_helperData->log("make array", 'ecollect-standard.log', $pref);

        $prefTxt = json_encode($pref);
        //file_put_contents("preferenceLog.html",$prefTxt);

        //file_put_contents("ecollectApi.html","before mp->create_preference<br>");
        $response = $mp->create_preference($pref);
        //file_put_contents("ecollectApi.html","AFTER mp->create_preference<br>",FILE_APPEND);
        $this->_helperData->log("create preference result", 'ecollect-standard.log', $response);
        if ($response['status'] == 200 || $response['status'] == 201) {
            if(isset($response['init_point'])){
                ...
                $this->_helperData->log("Array preference ok", 'ecollect-standard.log');
            }else{
                $array_assign = [
                ...
                $this->_helperData->log("Array preference error", 'ecollect-standard.log');
            }
        } else {
            $array_assign = [
                "message" => __('An error has occurred. Please refresh the page.'),
    ...
    public function makePreference()
    {
        ...
        $this->_eventManager->dispatch(
            'ecollect_standard_make_preference_before',
        ...
        if ($total_item > $order_amount || $total_item < $order_amount) {
            ...
            $arr['items'][] = [
                ...
                "category_id" => $this->_scopeConfig->getValue('payment/ecollect/category_id', \Magento\Store\Model\ScopeInterface::SCOPE_STORE),
                "quantity"    => 1,
                "unit_price"  => (float)$diff_price
            ];
            $this->_helperData->log("Total itens: " . $total_item, 'ecollect-standard.log');
            $this->_helperData->log("Total order: " . $order_amount, 'ecollect-standard.log');
            $this->_helperData->log("Difference add itens: " . $diff_price, 'ecollect-standard.log');
            $arr['amount'] = $order_amount + $diff_price;
        }
        else {
        ...
        if (!$customer->getId()) {
            ...
            $arr['payer']['dni'] = htmlentities($order->getData('customer_taxvat'));
        }
        ...
        if ($typeCheckout == 'redirect') {
            ...
            if (!$this->_scopeConfig->getValue(\ecollect\Core\Helper\Data::XML_PATH_USE_SUCCESSPAGE_MP, \Magento\Store\Model\ScopeInterface::SCOPE_STORE)){
                ...
                $arr['back_urls']['failure'] = $this->_urlBuilder->getUrl('ecollect/standard/failure');
            }
        }

        $arr['notification_url'] = $this->_urlBuilder->getUrl("ecollect/notifications/standard");
        ...
        $sponsor_id = $this->_scopeConfig->getValue('payment/ecollect/sponsor_id', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $this->_helperData->log("Sponsor_id", 'ecollect-standard.log', $sponsor_id);
        if (!empty($sponsor_id)) {
            $this->_helperData->log("Sponsor_id identificado", 'ecollect-standard.log', $sponsor_id);
    ...
    public function isAvailable(\Magento\Quote\Api\Data\CartInterface $quote = null)
    {
        ...
        $clientId = $this->_scopeConfig->getValue(\ecollect\Core\Helper\Data::XML_PATH_CLIENT_ID, \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $clientSecret = $this->_scopeConfig->getValue(\ecollect\Core\Helper\Data::XML_PATH_CLIENT_SECRET, \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $standard = (!empty($clientId) && !empty($clientSecret));

        //file_put_contents("ecollectlog.html","Core Model StandardPayment isAvailable parent = $parent clientId = $clientId secret = $clientSecret", FILE_APPEND);
        if (!$parent || !$standard) {
            return false;
        }

        $isValidClientCredentials = $this->_helperData->isValidClientCredentials($clientId, $clientSecret);
        //file_put_contents("ecollectlog.html","Core Model StandardPayment isAvailable valid credentials = $isValidClientCredentials", FILE_APPEND);

        return $isValidClientCredentials;*/
