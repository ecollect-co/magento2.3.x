...
/**
 * Core Model of ecollect plugin, used by all payment methods
 *
 * Class Core
 *
    ...
    extends \Magento\Payment\Model\Method\AbstractMethod
{
     ...
     * @param \ecollect\Core\Helper\Message\MessageInterface     $statusMessage
     * @param \ecollect\Core\Helper\Message\MessageInterface     $statusDetailMessage
    ...
    public function __construct(
        ...
        \ecollect\Core\Helper\Message\MessageInterface $statusMessage,
        \ecollect\Core\Helper\Message\MessageInterface $statusDetailMessage,
    ...
    protected function getCouponInfo($coupon, $coupon_code)
    {
        ...
        if ($coupon['status'] == 200) {
            $this->_coreHelper->log("Coupon applied. API response 200.", 'ecollect-custom.log');
        } else {
            $this->_coreHelper->log("Coupon invalid, not applied.", 'ecollect-custom.log');
        }
    ...
    public function makeDefaultPreferencePaymentV1($paymentInfo = [], $quote = null, $order = null)
    {
        ...
        $preference['notification_url'] = $this->_urlBuilder->getUrl('ecollect/notifications/custom');
        ...
        if (!empty($paymentInfo['coupon_code'])) {
            ...
            $this->_coreHelper->log("Validating coupon_code: " . $couponCode, 'ecollect-custom.log');

            $coupon = $this->validCoupon($couponCode);
            $this->_coreHelper->log("Response API Coupon: ", 'ecollect-custom.log', $coupon);
        ...
        $sponsorId = $this->_scopeConfig->getValue('payment/ecollect/sponsor_id', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $this->_coreHelper->log("Sponsor_id", 'ecollect-standard.log', $sponsorId);
        if (!empty($sponsorId)) {
            $this->_coreHelper->log("Sponsor_id identificado", 'ecollect-custom.log', $sponsorId);
     ...
     *
     * @return array
     * @throws \ecollect\Core\Model\Api\V1\Exception
    ...
    public function postPaymentV1($preference)
    {
        ...
        $this->_coreHelper->log("Access Token for Post", 'ecollect-custom.log', $this->_accessToken);

        //set sdk php ecollect
        $mp = $this->_coreHelper->getApiInstance($this->_accessToken);
        $response = $mp->post("/v1/payments", $preference);
        $this->_coreHelper->log("POST /v1/payments", 'ecollect-custom.log', $response);

        if ($response['status'] == 200 || $response['status'] == 201) {
            return $response;
        } else {
            $e = "";
            $exception = new \ecollect\Core\Model\Api\V1\Exception(new \Magento\Framework\Phrase($e), $this->_scopeConfig);
            ...
            $this->_coreHelper->log("erro post pago: " . $e, 'ecollect-custom.log');
            $this->_coreHelper->log("response post pago: ", 'ecollect-custom.log', $response);
    ...
    public function getPayment($order_id)
    {
        if (!$this->_clientId || !$this->_clientSecret) {
            $this->_clientId = $this->_scopeConfig->getValue(\ecollect\Core\Helper\Data::XML_PATH_CLIENT_ID, \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
            $this->_clientSecret = $this->_scopeConfig->getValue(\ecollect\Core\Helper\Data::XML_PATH_CLIENT_SECRET, \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        }
        $mp = $this->_coreHelper->getApiInstance($this->_clientId, $this->_clientSecret);
        //vmorales get ticket
        return $mp->get_payment($order_id);
    }

    public function getPaymentE($order_id, $pref)
    {

        ////file_put_contents("log.html","Core getPaymentE<br>",FILE_APPEND);
        if (!$this->_clientId || !$this->_clientSecret) {
            $this->_clientId = $this->_scopeConfig->getValue(\ecollect\Core\Helper\Data::XML_PATH_CLIENT_ID, \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
            $this->_clientSecret = $this->_scopeConfig->getValue(\ecollect\Core\Helper\Data::XML_PATH_CLIENT_SECRET, \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        }
        ...
        //file_put_contents("log.html","before mp->get_PaymentE<br>",FILE_APPEND);
        return $mp->get_paymentE($order_id, $pref);

        //file_put_contents("log.html","after mp->get_PaymentE<br>",FILE_APPEND);
    }
