    ...
    implements ObserverInterface
{
    ...
    protected function creditMemoRefundBeforeSave($order, $creditMemo)
    {
        ...
        if ($order->getExternalRequest()) {
            return; // si la peticion de crear un credit memo viene de ecollect, no hace falta mandar el request nuevamente
        }
    ...
    protected function checkRefundBasicData($paymentMethod, $paymentDate)
    {
        ...
        if (!$refundAvailable) {
            $this->_messageManager->addNoticeMessage(__('e-Collect refunds are disabled. The refund will be made through Magento'));
        ...
        if (!($paymentMethod == 'ecollect_standard' || $paymentMethod == 'ecollect_custom')) {
            $this->_messageManager->addErrorMessage(__('Order payment wasn\'t made by e-Collect. The refund will be made through Magento'));
    ...
    protected function sendRefundRequest($order, $creditMemo, $paymentMethod, $isTotalRefund)
    {
        ...
        if ($paymentMethod == 'ecollect_standard') {
        ...
        if ($response['status'] == 201 || $response['status'] == 200) {
            ...
            $this->_messageManager->addSuccessMessage(__('Refund made by e-Collect'));
        } else {
            $this->_messageManager->addErrorMessage(__('Failed to make the refund by e-Collect'));
    ...
    {
        throw new \Magento\Framework\Exception\LocalizedException(new \Magento\Framework\Phrase('e-Collect - Refund not made'));
    }
