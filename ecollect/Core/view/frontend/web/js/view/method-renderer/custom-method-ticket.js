...
define(
    ...
    function (Component,paymentService,paymentMethodList,getTotalsAction,$,fullScreenLoader, setAnalyticsInformation) {
        ...
        var configPayment = window.checkoutConfig.payment.ecollect_customticket;

        return Component.extend({
            defaults: {
                template: 'ecollect_Core/payment/custom_ticket',
                paymentReady: false
            },
            ...
            getData: function () {
                var dataObj = {
                    ...
                    'additional_data': {
                        ...
                        'total_amount':  TinyJ('#ecollect_checkout_custom_ticket .total_amount').val(),
                        'amount': TinyJ('#ecollect_checkout_custom_ticket .amount').val(),
                        'site_id': this.getCountry(),
                    }
                };
                if (configPayment != undefined) {
                    if (configPayment['discount_coupon'] == 1) {
                        dataObj.additional_data['ecollect-discount-amount'] = TinyJ('#ecollect_checkout_custom_ticket .ecollect-discount-amount').val();
                        dataObj.additional_data['coupon_code'] = TinyJ('#ecollect_checkout_custom_ticket #input-coupon-discount').val();
                    }
                }
